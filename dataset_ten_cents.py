    ...
    with open(csv_file) as fichier_csv:
        ...
        for row in reader:
            if float(row[1]) < 0:  # Remove negative values
                continue
            weight = int(math.ceil(float(row[1]) * 10))  # use 0,1$ as unit
            pourcentage = float(row[2])
            value = int(pourcentage / 100 * int(float(row[1]) * 10))
...
def items_in_dataset(h, w, wts, table, dataset_name, dataset_weight):
    ...
    while i > 0 and j > 0:
        if table[i][j] != table[i - 1][j]:
            print(f"{dataset_name[i]} pour un prix de {dataset_weight[i]/10} dollars.")
...
def main():
    ...
    CAPACITY = 5000  # The capacity is in dollar for a lesser precision
